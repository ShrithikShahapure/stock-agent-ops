"""SageMaker Training script: train all child LSTM models via transfer learning.

Runs inside a SageMaker PyTorch training container.
Loads the parent model from the 'parent' input channel, reads each child
ticker's CSV from the 'training' channel, applies transfer learning
(freeze LSTM, retrain FC), and saves all child model artifacts.
"""
...
class StockDataset(Dataset):
    def __init__(self, df: pd.DataFrame, scaler: StandardScaler,
        ...
        if not self.samples:
            raise ValueError(f"No valid samples created from {len(df)} rows")
...
def main():
    ...
    parser.add_argument("--child-tickers", type=str, required=True,
                        help="Comma-separated list of child tickers")
    ...
    parser.add_argument("--transfer-strategy", type=str, default="freeze",
                        choices=["freeze", "fine_tune"])
    ...
    parent_dir = os.environ.get("SM_CHANNEL_PARENT", "/opt/ml/input/data/parent")
    ...
    parent_model_path = os.path.join(parent_dir, f"{safe_parent}_parent_model.pt")
    print(f"Loading parent model from {parent_model_path}")

    parent_state = torch.load(parent_model_path, map_location=device, weights_only=True)
    ...
    for ticker in child_tickers:
        print(f"\n{'='*60}")
        print(f"Training child model: {ticker}")
        print(f"{'='*60}")
        ...
        if not os.path.exists(csv_path):
            alt_name = ticker.replace("-", "_")
            csv_path = os.path.join(data_dir, f"{alt_name}.csv")
        if not os.path.exists(csv_path):
            # Try original ticker name with dash
            csv_path = os.path.join(data_dir, f"{ticker.replace('^', '')}.csv")

        if not os.path.exists(csv_path):
            print(f"  WARNING: Data file not found for {ticker}, skipping")
        ...
        try:
            ...
            # Create model and load parent weights
            model = LSTMModel(
            ...
            if args.transfer_strategy == "freeze":
                print(f"  Strategy: freeze LSTM layers, retrain FC")
                for name, param in model.named_parameters():
                    if "lstm" in name:
                        param.requires_grad = False
            else:
                print(f"  Strategy: fine-tune all layers")

            model = fit_model(model, train_loader, val_loader,
                              epochs=args.epochs, lr=args.lr, device=device)

            # Save child artifacts
            child_dir = os.path.join(model_dir, safe_name)
            os.makedirs(child_dir, exist_ok=True)

            child_model_path = os.path.join(child_dir, f"{safe_name}_child_model.pt")
            child_scaler_path = os.path.join(child_dir, f"{safe_name}_child_scaler.pkl")
            torch.save(model.state_dict(), child_model_path)
            joblib.dump(scaler, child_scaler_path)
            print(f"  Saved model to {child_model_path}")

            # Evaluate
            split_idx = int(len(df) * 0.8)
            val_df = df.iloc[split_idx:]
            metrics = evaluate(model, val_df, scaler,
                               args.context_len, args.pred_len, device)
            all_metrics[ticker] = metrics
            print(f"  Metrics: {json.dumps(metrics, indent=2)}")

        except Exception as e:
            print(f"  ERROR training {ticker}: {e}", file=sys.stderr)
            failed.append(ticker)

    # Save aggregate metrics
    results = {
        "children": all_metrics,
        "failed": failed,
        "total": len(child_tickers),
        "succeeded": len(child_tickers) - len(failed),
    ...
    print(f"\n{'='*60}")
    print(f"Child training complete: {len(child_tickers) - len(failed)}/{len(child_tickers)} succeeded")
    if failed:
