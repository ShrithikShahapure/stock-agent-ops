...
def _safe_mlflow_log(func, *args, **kwargs):
    """Safely call MLflow logging functions - ignore errors."""
...
def train_parent() -> Dict:
    ...
    if MLFLOW_AVAILABLE:
        try:
            mlflow.start_run(run_name=f"Parent_Training_{ticker}")
            run_id = mlflow.active_run().info.run_id
            _safe_mlflow_log(mlflow.log_params, {
            ...
            logger.warning(f"MLflow run start skipped: {e}")
    ...
    try:
        ...
        for k, v in metrics.items():
            _safe_mlflow_log(mlflow.log_metric, k, v)
        _safe_mlflow_log(mlflow.log_artifact, torch_path, "torch_model")
        _safe_mlflow_log(mlflow.log_artifact, scaler_path, "scaler")

        logger.info(f"✅ Parent {ticker} trained successfully")
        return {"ticker": ticker, "run_id": run_id, "metrics": metrics}
    except Exception as e:
...
def train_child(ticker: str) -> Dict:
    ...
    if MLFLOW_AVAILABLE:
        try:
            mlflow.start_run(run_name=f"Child_Training_{ticker}")
            run_id = mlflow.active_run().info.run_id
            _safe_mlflow_log(mlflow.log_params, {
                "context_len": cfg.context_len,
                "pred_len": cfg.pred_len,
