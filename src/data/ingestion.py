...
def fetch_ohlcv(ticker: str, start: str = Config().start_date, end: Optional[str] = None) -> pd.DataFrame:
    ...
    try:
        ...
        print(f"Fetched {len(df)} rows for {ticker}", file=sys.stderr)
        ...
        try:
            ...
            print(f"✅ Saved features to {data_path}", file=sys.stderr)
            ...
            print("🔄 Running Feast apply...", file=sys.stderr)
            subprocess.run(["feast", "apply"], cwd=repo_path, check=True, capture_output=True)
            
            print("🔄 Running Feast materialization...", file=sys.stderr)
            ...
            print("✅ Feast features materialized to Redis", file=sys.stderr)

        except subprocess.CalledProcessError as e:
            print(f"⚠️ Feast command failed: {e.stderr.decode() if e.stderr else e}", file=sys.stderr)
        except Exception as e:
            print(f"⚠️ Feast ingestion failed: {e}", file=sys.stderr)
